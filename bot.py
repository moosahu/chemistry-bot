...
def main_menu_callback(update: Update, context: CallbackContext) -> int:
    ...
    if data == \'menu_info\':
        ...
        # We might need to explicitly return the INFO_MENU state if not using nested conv handlers
        # For now, let info_handlers handle the transition via its entry point
        # If show_info_menu is imported and called here, it might work too, but let's rely on the handler structure
        logger.warning("Transitioning to info menu - should be handled by info_menu_conv_handler")
        # Returning a state here might conflict if info_menu_conv_handler is separate
        # Let's just edit the message slightly and assume the other handler picks it up.
        # This part is tricky without knowing the exact dispatcher setup.
        # Fallback: Call show_info_menu directly if it's imported
        try:
            from info_menu_function import show_info_menu
            return show_info_menu(update, context)
        except ImportError:
             logger.error("Cannot call show_info_menu directly.")
