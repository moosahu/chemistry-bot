...
class QuizLogic:
    ...
    async def send_question(self, chat_id: int, user_id: int) -> None:
        ...
        total_questions_val = quiz_data["total_questions"]
        header = f"<b>السؤال {q_idx + 1} من {total_questions_val}:</b>\n"
        full_question_text = header + question_text_main
        reply_markup = self._build_question_keyboard(question, quiz_id, q_idx)
        last_msg_id = quiz_data.get("last_message_id")
        new_message_id = await self._send_or_edit_message(
            chat_id=chat_id, text=full_question_text, reply_markup=reply_markup,
            photo_url=question_image_url, current_message_id=last_msg_id
        )
        if new_message_id:
        ...
        if ENABLE_QUESTION_TIMER:
            ...
            timer_context_data = {"chat_id": chat_id, "user_id": user_id, "quiz_id": quiz_id, "question_index": q_idx}
            self.context.job_queue.run_once(
                question_timer_callback, timedelta(seconds=QUESTION_TIMER_SECONDS),
                data=timer_context_data, name=job_name, chat_id=chat_id, user_id=user_id
            )
    ...
    async def start_quiz(self, update: Update) -> int:
        ...
        if not quiz_selection or "type" not in quiz_selection or "count" not in quiz_selection or "endpoint" not in quiz_selection:
            sel_str = str(quiz_selection)
            logger.error(f"[QuizLogic] start_quiz for user {user_id} without complete quiz_selection: {sel_str}")
        ...
        if num_questions <= 0:
            sel_str = str(quiz_selection)
            logger.warning(f"[QuizLogic] No questions available ({max_available}) for user {user_id} for selection {sel_str}. Aborting quiz.")
        ...
        if questions_endpoint == "random_api":
            ...
            if api_response == "TIMEOUT" or not isinstance(api_response, list):
                resp_str = str(api_response)
                logger.error(f"[QuizLogic] Failed to fetch questions from {questions_endpoint} for user {user_id}. Response: {resp_str}")
        ...
        for q_data in quiz_questions_raw:
            ...
            if transformed and (transformed.get("options") or transformed.get("correct_answer") is not None):
                ...
                q_data_str = str(q_data)
                logger.warning(f"[QuizLogic] Skipping invalid question after transformation: {q_data_str}")
        ...
        quiz_type_display_val = quiz_selection.get("type_display_name", "اختبار عام")
        quiz_scope_display_val = quiz_selection.get("scope_display_name", "")
        self.user_data["current_quiz"] = {
            "quiz_id": quiz_id, "questions": quiz_questions_transformed,
            "total_questions": num_questions, "current_question_index": 0,
            "answers": [None] * num_questions, "score": 0, "start_time": datetime.now(),
            "finished": False, "last_message_id": None,
            "quiz_type_display": quiz_type_display_val,
            "quiz_scope_display": quiz_scope_display_val,
    ...
    async def handle_answer(self, update: Update, chosen_option_index: int) -> int:
        ...
        if options_list:
            ...
            correct_answer_text_val = question_data.get(correct_opt_text_key)
            correct_answer_image_val = question_data.get(correct_opt_image_key)
            correct_answer_text_or_image = correct_answer_text_val or ("صورة صحيحة" if correct_answer_image_val else "غير محدد")
        ...
        last_msg_id = quiz_data.get("last_message_id")
        if last_msg_id:
             await safe_edit_message_text(self.bot, chat_id, last_msg_id, text=feedback_text, reply_markup=None)
             await asyncio.sleep(FEEDBACK_DELAY)
        else:
            await safe_send_message(self.bot, chat_id, text=feedback_text)
            await asyncio.sleep(FEEDBACK_DELAY)

        quiz_id_val = quiz_data["quiz_id"]
        job_name = f"qtimer_{chat_id}_{user_id}_{quiz_id_val}_{q_idx}"
        remove_job_if_exists(job_name, self.context)

        quiz_data["current_question_index"] += 1
        total_questions_val = quiz_data["total_questions"]
        if quiz_data["current_question_index"] < total_questions_val:
            await self.send_question(chat_id, user_id)
            return TAKING_QUIZ
        else:
    ...
    async def handle_skip_question(self, update: Update, timed_out: bool = False, error_occurred: bool = False) -> int:
        ...
        if q_idx >= total_questions_val or quiz_data["answers"][q_idx] is not None:
        ...
        last_msg_id = quiz_data.get("last_message_id")
        if last_msg_id:
            await safe_edit_message_text(self.bot, chat_id, last_msg_id, text=skip_message, reply_markup=None)
            await asyncio.sleep(FEEDBACK_DELAY)
        else:
    ...
    async def finish_quiz(self, update: Update, from_answer: bool = False, from_skip: bool = False) -> int:
        ...
        start_time_val = quiz_data["start_time"]
        duration_seconds = (end_time - start_time_val).total_seconds()
        score = quiz_data["score"]
        total_q = quiz_data["total_questions"]
        percentage = (score / total_q * 100) if total_q > 0 else 0
        quiz_type_display_val = quiz_data.get("quiz_type_display", "N/A")
        quiz_scope_display_val = quiz_data.get("quiz_scope_display", "N/A")
        quiz_id_val = quiz_data["quiz_id"]

        try:
            DB_MANAGER.add_quiz_result(
                user_id=user_id, quiz_type=quiz_type_display_val, quiz_scope=quiz_scope_display_val,
                score=score, total_questions=total_q, percentage=percentage,
                duration_seconds=int(duration_seconds), quiz_timestamp=start_time_val
            )
            logger.info(f"[QuizLogic] Quiz {quiz_id_val} results saved for user {user_id}.")
        ...
        last_msg_id = quiz_data.get("last_message_id")
        if last_msg_id:
            await safe_delete_message(self.bot, chat_id, last_msg_id)
...
async def question_timer_callback(context: CallbackContext):
    ...
    current_quiz_id = current_quiz_data.get("quiz_id") if current_quiz_data else None
    current_q_idx = current_quiz_data.get("current_question_index") if current_quiz_data else -1 # Use -1 if no index
    is_finished = current_quiz_data.get("finished") if current_quiz_data else True # Assume finished if no data
    answer_at_idx = current_quiz_data["answers"][question_index_timed_out] if current_quiz_data and question_index_timed_out < len(current_quiz_data.get("answers",[])) else True # Assume answered if index out of bounds or no answers array

    if (current_quiz_data and 
        current_quiz_id == quiz_id and
        current_q_idx == question_index_timed_out and 
        not is_finished and 
        answer_at_idx is None):
        ...
        class DummyUpdate:
            class DummyEffectiveUser: id = user_id
            class DummyEffectiveChat: id = chat_id
