...
async def stats_menu_callback_handler(update: Update, context: CallbackContext):
    ...
    await query.edit_message_text(text=f"اختر الفترة الزمنية لـ: {stat_category_base.replace("_", " ").title()}", reply_markup=reply_markup)
...
async def send_actual_stats(update: Update, context: CallbackContext, stat_category: str, time_filter: str):
    ...
    # Calls to admin_logic are assumed to be synchronous for now.
    # If admin_logic functions become async, they would need await here.
