...
async def select_quiz_type(update: Update, context: CallbackContext) -> int:
    ...
    if quiz_type_key == QUIZ_TYPE_ALL:
        ...
        keyboard = create_question_count_keyboard(max_questions, quiz_type_key, unit_id="all")        await safe_edit_message_text(context.bot, chat_id=query.message.chat_id, message_id=query.message.message_id, text=f"""اختر عدد الأسئلة لاختبار \n'{quiz_type_display_name}':""", reply_markup=keyboard)
