...
class DatabaseManager:
    ...
    # --- NEWLY ADDED/FIXED ADMIN FUNCTIONS ---
    def get_average_quizzes_per_active_user(self, time_filter="all") -> float:
        ...
        total_completed_quizzes = self.get_total_quizzes_count(time_filter)
        active_users = self.get_active_users_count(time_filter)
        
        if active_users > 0:
            avg_quizzes = total_completed_quizzes / active_users
        else:
            avg_quizzes = 0.0
        logger.info(f"[DB Admin Stats V17] Avg quizzes per active user ({time_filter}): {avg_quizzes} (Total Quizzes: {total_completed_quizzes}, Active Users: {active_users})")
        return avg_quizzes
    ...
    def get_overall_average_score(self, time_filter="all") -> float:
        ...
        # This function is an alias or wrapper for get_average_score_percentage as per error log analysis
    ...
    def get_quiz_completion_rate_stats(self, time_filter="all") -> dict:
        ...
        # Get total completed quizzes (uses completed_at for time filter)
        total_completed = self.get_total_quizzes_count(time_filter)
        
        # Get total started quizzes (uses start_time for time filter)
        ...
        raw_result_started = self._execute_query(query_started, fetch_one=True)
        total_started = raw_result_started["total_started"] if raw_result_started and "total_started" in raw_result_started else 0
        
        if total_started > 0:
            completion_rate_percentage = (total_completed / total_started) * 100.0
        else:
            completion_rate_percentage = 0.0
            
        stats = {
            "completed_count": total_completed,
            "attempted_count": total_started, # "Attempted" here means started
            "completion_rate_percentage": completion_rate_percentage
        }
        logger.info(f"[DB Admin Stats V17] Quiz completion stats ({time_filter}): {stats}")
        return stats
    # --- END OF NEWLY ADDED/FIXED ADMIN FUNCTIONS ---

    def get_detailed_question_stats(self, time_filter="all"):
        logger.info(f"[DB Admin Stats V17 Final Fix] Fetching detailed question stats for filter: {time_filter}")
        ...
        logger.info(f"[DB Admin Stats V17 Final Fix] Raw results for detailed_question_stats ({time_filter}): {raw_results}")
        ...
        if raw_results:
            ...
            logger.info(f"[DB Admin Stats V17 Final Fix] Processed {len(processed_results)} question stats for filter: {time_filter}")
            return processed_results
        else:
            logger.warning(f"[DB Admin Stats V17 Final Fix] No detailed question stats found for filter: {time_filter}. Returning empty list.")
            return []
