...
def transform_api_question(api_question: dict) -> dict | None:
    ...
    try:
        # --- Basic Fields ---
        question_id = api_question.get('id') or api_question.get('question_id') # Allow both 'id' and 'question_id'
        question_text = api_question.get('question_text')
        image_url = api_question.get('image_url')
        ...
        if 'options' in api_question and isinstance(api_question['options'], list):
            # Assuming options list contains text strings
            api_options = api_question['options']
            for i in range(min(len(api_options), 4)):
                options_list[i] = api_options[i]
        else:
            # Try fetching individual option fields
            options_list[0] = api_question.get('option1') or api_question.get('option_1')
            options_list[1] = api_question.get('option2') or api_question.get('option_2')
            options_list[2] = api_question.get('option3') or api_question.get('option_3')
            options_list[3] = api_question.get('option4') or api_question.get('option_4')

        # --- Correct Answer --- 
        # Expecting a zero-based index (0, 1, 2, 3)
        correct_answer_index = api_question.get('correct_answer') # Could be index or text
        correct_option_index = api_question.get('correct_option_index') # Prefer explicit index
        ...
        if correct_option_index is not None and isinstance(correct_option_index, int) and 0 <= correct_option_index < 4:
            ...
            if isinstance(correct_answer_index, int) and 0 <= correct_answer_index < 4:
                ...
                try:
                    ...
                    # Corrected logging string
                    logger.warning(f"[API_TRANSFORM] Correct answer text '{correct_answer_index}' not found in options: {options_list} for q_id: {question_id}")
                    final_correct_index = None # Cannot determine index

        # --- Validation --- 
        if not question_id or not question_text:
            # Corrected logging string
            logger.warning(f"[API_TRANSFORM] Missing question_id or question_text in API data: {api_question}")
            return None
        if final_correct_index is None:
            # Corrected logging string
            logger.warning(f"[API_TRANSFORM] Could not determine valid correct_answer index (0-3) for q_id: {question_id}. Data: {api_question}")
            return None
        if all(opt is None for opt in options_list):
             # Corrected logging string
             logger.warning(f"[API_TRANSFORM] All options are None for q_id: {question_id}. Data: {api_question}")
        ...
        internal_question = {
            ...
            "option1_image": api_question.get('option1_image'),
            "option2_image": api_question.get('option2_image'),
            "option3_image": api_question.get('option3_image'),
            "option4_image": api_question.get('option4_image'),
        }
        # Corrected logging string
